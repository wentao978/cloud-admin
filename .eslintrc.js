module.exports = {
    ...
    'rules': {
        ...
        "no-console":process.env.NODE_ENV === "production" ? "error" : "off", //allow是个字符串数组，包含允许使用的console 对象的方法
        ...
        "no-debugger": process.env.NODE_ENV === "production" ? "error" : "off", //禁用 debugger , debugger 语句用于告诉 JavaScript 执行环境停止执行并在代码的当前位置启动调试器。 如:function isTruthy(x) { debugger; return Boolean(x);}
